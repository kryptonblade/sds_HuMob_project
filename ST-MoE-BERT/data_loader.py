...
# from datasets import load_dataset  # Commented out as it's unused
from predict import generate_sequence
from utils import *


def morton_encode(x, y, bits=8):
    """Interleave bits for Morton / Z-order encoding."""
...
class MobTimeSeriesDatasetWImputation(Dataset):
    def __init__(self, dataset, input_seq_length, predict_seq_length, subsample=False, subsample_number=100):
        ...
        dataset['label'] = morton_encode(dataset['x'].to_numpy(dtype=np.int64) - 1,
                                         dataset['y'].to_numpy(dtype=np.int64) - 1)
    ...
    def prepare_sequences(self):
        ...
        for uid in tqdm(self.uids, desc="Preparing sequences with imputation"):
    ...
    def generate_sequence(self, data_by_day, days, uid):
        ...
        for d in days:
            ...
            full_day_x = []
            full_day_y = np.full(48, PAD_LOCATION)  # default value for missing time slots
            
            for t in range(48):
                delta_t = t - previous_t
                full_day_x.append([d, t, uid, d % 7, t % 24, delta_t])
                previous_t = t
            
            full_day_x = np.array(full_day_x)
        ...
        # seq_x shape is (75*48, 6), seq_y shape is (75*48,)
    ...
    def __getitem__(self, idx):
        '''
        The format of seq_feature is (day, time, uid, day_of_week, time_of_day, delta_t)
...
class MobTimeSeriesDataset(Dataset):
    ...
    def prepare_sequences(self, look_back_len=24, multiple=5):
        ...
        for idx, uid in enumerate(tqdm(self.uids, desc="Preparing sequences")):
            uid_df = self.dataset[self.dataset['uid'] == uid]
...
def split_df_by_uid(city, test_size=0.1, random_seed=None, subsample=False, subsample_number=100):
    ...
    # Adjust split ratios for subsampled data
    if subsample and len(uids) < 3000:
        # For small subsampled datasets, use different ratios
        generate_count = min(200, len(uids) // 5)  # 20% for generation
        test_count = min(100, len(uids) // 10)     # 10% for testing
        
        generate_uid_list = uids[-generate_count:]
        generate_df = df[df['uid'].isin(generate_uid_list)]
        
        remain_df = df[df['x'] != 999]
        remaining_uids = [uid for uid in uids if uid not in generate_uid_list]
        
        _, test_uids = train_test_split(remaining_uids, test_size=test_count, random_state=random_seed)
    else:
        # Original logic for full datasets
