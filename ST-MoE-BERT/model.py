...
class Transformer(nn.Module):
    def __init__(self, day_embedding_dim=16, time_embedding_dim=16, day_of_week_dim=8, user_embedding_dim=64, num_users=100,
                 model_dim=512, n_heads=8, num_layers=6, dropout=0.3, fc_layers=2, num_classes=NUM_LOCATION_IDS):
        '''
...
class MobilityBERT(nn.Module):
    ...
    def __init__(self, num_location_ids, hidden_size, hidden_layers, attention_heads,
        ...
        self.day_embedding = nn.Embedding(1000, day_embedding_size)
        self.time_embedding = nn.Embedding(1000, time_embedding_size)
        self.day_of_week_embedding = nn.Embedding(1000, day_of_week_embedding_size)
        self.weekday_embedding = nn.Embedding(1000, weekday_embedding_size)
        self.location_embedding = nn.Embedding(num_location_ids, location_embedding_size)
        self.user_embedding = nn.Embedding(1000, hidden_size // 4)  # Add user embedding

        # Input concatenation and projection
        input_size = day_embedding_size + time_embedding_size + day_of_week_embedding_size + location_embedding_size + weekday_embedding_size + (hidden_size // 4) + 1
    ...
    def forward(self, input_seq_feature, historical_locations, predict_seq_feature):
        """
        The forward method processes both historical and future input features through embeddings, 
        BERT, and a custom attention layer, with safety clamping for out-of-bounds indices.
        """
        # 1. Clamp ALL input features to stay within the embedding size (1000)
        # This handles day, time, day_of_week, and weekday embeddings
        safe_input_seq = torch.clamp(input_seq_feature, 0, 999)
        safe_predict_seq = torch.clamp(predict_seq_feature, 0, 999)
        
        # 2. Handle locations separately (must be < num_location_ids)
        # If historical_locations contains PAD_LOCATION, it's mapped to 0 for the embedding layer
        safe_hist_locations = torch.where(
            historical_locations == PAD_LOCATION, 
            torch.zeros_like(historical_locations), 
            historical_locations
        )

        # Map features from the SAFE versions - include uid feature
        historical_days, historical_times, historical_uid, historical_day_of_weeks, hist_weekday, hist_delta = map(
            lambda x: safe_input_seq[:, :, x], range(6))
        
        future_days, future_times, future_uid, future_day_of_weeks, future_weekday, future_delta = map(
            lambda x: safe_predict_seq[:, :, x], range(6))
        
        # Embedding lookups now use safe clamped indices
        hist_day_emb = self.day_embedding(historical_days)
        hist_time_emb = self.time_embedding(historical_times)
        hist_uid_emb = self.user_embedding(historical_uid)
        ...
        hist_loc_emb = self.location_embedding(safe_hist_locations)
        
        historical_input = self.layer_norm(torch.cat([
            hist_day_emb, hist_time_emb, hist_uid_emb, hist_dow_emb, hist_weekday_emb, hist_delta, hist_loc_emb
        ], dim=-1))
        
        # Future inputs also use safe features
        future_input = self.layer_norm(torch.cat([
            self.day_embedding(future_days),
            self.time_embedding(future_times),
            self.user_embedding(future_uid),
            self.day_of_week_embedding(future_day_of_weeks),
            self.weekday_embedding(future_weekday),
            future_delta.unsqueeze(-1).float(),
            torch.zeros_like(hist_loc_emb)[:, :future_days.size(1)]
        ], dim=-1))
...
class MixtureOfExperts(nn.Module):
    ...
    def __init__(self, num_experts, input_size, output_size):
        ...
        # Initialize gate with smaller weights to prevent extreme confidence
    ...
    def forward(self, x):
        ...
        x_reshaped = x.reshape(batch_size * seq_len, -1)  # Flatten input for processing by experts

        # Process input through all experts
        expert_outputs = torch.stack([expert(x_reshaped) for expert in self.experts], dim=1)
        
        # Compute gating probabilities with temperature for better exploration
        gate_logits = self.gate(x_reshaped)
        
        # Add noise to prevent collapse during training
        if self.training:
            gate_logits = gate_logits + torch.randn_like(gate_logits) * 0.5
        
        temperature = 2.0
        gate_probs = nn.functional.softmax(gate_logits / temperature, dim=1)
        ...
        if self.training:
            ...
            load_balance_loss = nn.functional.kl_div(
                (expert_usage + 1e-9).log(), target, reduction='batchmean'
            )
            
            # 🔥 NEW: entropy regularization
            entropy = -(gate_probs * torch.log(gate_probs + 1e-9)).sum(dim=1).mean()
            
            aux_loss = load_balance_loss - 0.01 * entropy
...
class MobilityBERTMoE(nn.Module):
    ...
    """
    def __init__(self, num_location_ids=NUM_LOCATION_IDS, hidden_size=256, hidden_layers=24, attention_heads=16,
    ...
    def forward(self, input_seq_feature, historical_locations, predict_seq_feature):
        """
        Processes sequences through MoE with dynamic routing and safety clamping.
        """
        # Safety clamping for all embedding layers
        safe_input_seq = torch.clamp(input_seq_feature, 0, 999)
        safe_predict_seq = torch.clamp(predict_seq_feature, 0, 999)
        ...
        # Slice features from safe tensors - match MobilityBERT exactly
        historical_days, historical_times, historical_uid, historical_day_of_weeks, hist_weekday, hist_delta = map(
            lambda x: safe_input_seq[:, :, x], range(6))
        ...
        future_uid_emb = self.user_embedding(future_uid)
        ...
        future_input = self.layer_norm(torch.cat([
            future_day_emb, future_time_emb, future_uid_emb, future_dow_emb, future_weekday_emb, 
            future_delta, torch.zeros_like(hist_loc_emb)[:, :future_seq_length]
        ], dim=-1))
        ...
        moe_output, aux_loss = self.moe(outputs.last_hidden_state[:, -future_days.size(1):])
